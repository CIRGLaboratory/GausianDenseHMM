...
def parse_args():
    ...
    return args.disruption,  args.param
...
def run_experiment(dsize, disruption, param, simple_model=True, l_fixed=True):
    ...
    with open(f"{RESULT_DIR}/best_result_hmmlearn_d{disruption}_p{param}_s{s}_T{T}_n{n}_simple_model{simple_model}_l{l_fixed}.json", "w") as f:
        json.dump(best_result, f, indent=4)
    return 0


if __name__ == "__main__":
    disruption, param = parse_args()

    Path(RESULT_DIR).mkdir(exist_ok=True, parents=True)

    run_experiment(data_sizes[0], disruption, param, simple_model=True, l_fixed=True)
