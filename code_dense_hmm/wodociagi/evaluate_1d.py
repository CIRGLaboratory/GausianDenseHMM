...
interval = 24 * 7 * 6
t = time.localtime()
month_len = np.cumsum([0, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31, 31, 28])
...
def cluster_weekly(model_log, Y_true, Y_test, states, states_test, target, target_test):
    ...
    kmeans_Y = KMeans(n_clusters=2).fit(Y_true.reshape(-1, 24*6*1))
    ...
    result = {
        "water_demand": {
            ...
            "test_ACC": acc_perm((kmeans_Y.predict(Y_test.reshape(-1, 24*6*1)) == target_test).mean())
        },
        "state_IDs": {
...
def classify_working(k, model_log, Y_true, Y_test, states, states_test, target, target_test):
    ...
    knn_Y = KNeighborsClassifier(n_neighbors=k).fit(Y_true.reshape(-1, 24 * 6 * 1), target)
    ...
    knn_preds_Y = knn_Y.predict(Y_true.reshape(-1, 24 * 6 * 1))
    knn_preds_Y_test = knn_Y.predict(Y_test.reshape(-1, 24 * 6 * 1))
...
def classify_weekday(k, model_log, Y_true, Y_test, states, states_test, target_w, target_w_test):
    ...
    knn_Y = KNeighborsClassifier(n_neighbors=k).fit(Y_true.reshape(-1, 24 * 6 * 1), target_w)
