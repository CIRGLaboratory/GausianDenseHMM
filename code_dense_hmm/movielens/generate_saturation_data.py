...
for i in tqdm(range(100)):
    ...
    selected = movies_available.loc[movies_available.i_id.isin(genres.index[genres[genre_tmp]]), :].groupby("u_id").apply(lambda df: pd.Series(np.random.choice(df.sort_values("pred")[-100:].index, 10, False))).melt().value
    ...
    def task(j, new_scores=new_scores, i=i):
        ...
        saturation = generate_saturation(ratings, movies_available, i * 10 + j, new_scores_tmp)
        return saturation


    movies_available.drop("pred", axis=1, inplace=True)
    with Pool(nodes=10) as pool:
