...
def get_params(d_, n_):
    ...
    covars_ = np.random.uniform(0.5, 2.5, (n_, d_))
...
def eval_multiple(n_, d_, T_):
    ...
    Y = norm.cdf(Y, loc=means[X],  scale=covars[X]) * np.sqrt(10) * covars[X] + means[X]
...
if __name__ == "__main__":
    n, d, T = parse_args()
    t = time.localtime()
    result_dir = f"../../data/benchmark_artificial-{t.tm_year}-{t.tm_mon}-{t.tm_mday}-full-uniform"
